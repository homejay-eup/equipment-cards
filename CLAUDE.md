...
- **2026-09-23 Step 46：新增「標準售價」頁籤——已上線（commit `2260dbf`，已 push main；SQL＋6 筆初始資料已於正式 Supabase 執行）**：主機／設備整機銷售方案做成「依產品」查閱頁籤（左清單右詳情、固定價格欄位＋其他費用、保留歷史版本、所有登入者可看、`edit_quotes` 可編輯、手動＋CSV 批次匯入）。規格 `_管理/01_equipment-cards/specs/step46-standard-prices.md`，過程見 `_管理/00_執行紀錄.md` Step 46 條目。**待辦**：實機測試；DMS 168「平台費加收 40」單位待使用者確認。
