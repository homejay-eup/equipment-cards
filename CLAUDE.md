...
- **目前 git HEAD**：`e64ad72`（Step 45 料卡備註支援貼圖/貼表格，已 push main；待正式 Supabase 執行新 SQL + 實機測試）
- **2026-09-23 Step 46：新增「標準售價」頁籤——已執行完成，未 commit（分支 `feat/step46-standard-prices`）**：主機／設備整機銷售方案做成「依產品」查閱頁籤（左清單右詳情、固定價格欄位＋其他費用、保留歷史版本、所有登入者可看、`edit_quotes` 可編輯、手動＋CSV 批次匯入）。規格 `_管理/01_equipment-cards/specs/step46-standard-prices.md`，過程見 `_管理/00_執行紀錄.md` Step 46 條目。**待辦**：① 正式 Supabase 執行 `_開發檔案/sql/step46-standard-prices.sql`；② commit/push；③ 實機測試。
- **2026-09-08 Step 45：料卡「備註」欄位支援貼圖/貼表格——已執行完成，已 commit/push，待正式 Supabase 執行新 SQL**：以使用者提供的任務板「更新紀錄」截圖為對照，完全重用 Step 41/42 建好的複合內容機制（`RichContentEditor`/`RichContentView`/`RichTable`、`useUpdateAttachmentUpload`、`validateRichContent`），未修改這些共用元件本身。依 `data`→`frontend`→`tester`→`reviewer` 標準順序委派。`equipment_cards` 新增 `notes_image_urls`/`notes_table_data`，新增 `POST /api/cards/notes-signature` 簽名端點（不綁 equipment_id、不查部門，folder 固定 `equipment-cards/card-notes`），`POST /api/cards`／`PATCH /api/cards/[id]` 改用 `validateRichContent()` 驗證這兩欄位（PATCH 沿用既有 `edit_card_notes` 權限，三欄位任一有送到才整批驗證寫入，避免只改文字時清空既有圖片/表格）。`CardFormDialog.tsx` 備註欄位換成 `RichContentEditor`；`CardDetailDialog.tsx` 兩處備註顯示換成 `RichContentView`＋新增燈箱（`UpdateImageLightbox`，比照任務卡）。`tester` 抓到 1 個阻塞問題（`CardDetailDialog.tsx` 對新欄位缺少 `?? []`/`?? null` 防禦，SQL migration 執行前正式 DB 沒有這兩欄位會直接讓細節頁崩潰）已由主 session 直接修正並重跑 build/lint 通過；`reviewer` 通過，3 項非阻塞建議（孤兒圖片、寫法風格不一致、複合內容上限未客製化）不處理。完整過程見 `_管理/00_執行紀錄.md` Step 45 條目。**待辦**：① SQL migration `_開發檔案/sql/step45-card-notes-richcontent.sql` 在正式 Supabase 執行；② commit/push main；③ 實機測試新增/編輯料卡備註貼圖貼表格、細節頁顯示與燈箱。
...
- **2026-08-26 任務板 2 項獨立小修正（不算 Step，已 commit）**：① 更新紀錄新增輸入框從 `rows=2` 加高到 `rows=10`；② `TrackerClient.tsx` 拖曳卡片新增靠邊自動捲動（`dragClientYRef` + rAF 迴圈），解決欄位卡片一多、拖曳目標超出可視範圍要反覆重新拖曳的問題。commit `ae65a74`。
- **2026-08-26 Step 42：任務「說明」欄位支援貼圖/貼表格 + 更新紀錄支援修改——已執行完成，待 commit + 正式 Supabase 執行新 SQL**：規格 `_管理/01_equipment-cards/specs/step42-issue-description-richtext-and-update-edit.md`。決策：更新紀錄修改範圍文字＋圖片＋表格都能改、修改後直接顯示最後修改時間（不加「已編輯」標記）、「說明」欄位貼圖/貼表格適用編輯＋新增任務兩個 dialog（因此新增不依賴 issue id、只依賴部門的通用簽名端點）。依 `data`→`frontend`→`tester`→`reviewer` 標準順序委派，主 session 每階段皆逐檔核對程式碼＋重跑 `npm run build`。`data` agent 新增 `issues.description_image_urls`/`description_table_data`、`issue_updates.updated_at`，把 Step 41 既有驗證邏輯抽成共用函式 `src/lib/richContentValidation.ts`；CodeGraph blast radius 覆核確認這次規格準確、無誤植。`frontend` agent 把 `IssueDetailDialog.tsx` 內嵌邏輯抽成 `src/components/tracker/` 5 個共用元件供三個 dialog 共用，新增更新紀錄「編輯」按鈕。`tester` 發現 1 個非阻塞 UX 缺口（切換編輯目標會悄悄蓋掉未儲存內容），`reviewer` 判斷需要修並直接修正（改成停用其他項目的編輯/刪除按鈕，非隱藏）；另提出 7 項非阻塞建議（皆 Step 41 既有技術債，不處理）。完整過程見 `_管理/00_執行紀錄.md` 對應條目。**待辦**：① SQL migration `_開發檔案/sql/step42-issue-description-and-update-edit.sql` 在正式 Supabase 執行；② commit/push main；③ 實機測試。
- **2026-08-26 Step 43：5 項需求已執行完成並上線**：依 `data`→`frontend`→`tester`→`reviewer` 標準順序委派，過程中 `tester` 抓到 1 個 High（`EquipmentQuickPick.tsx` 改 `createPortal` 後 outside-click 判斷沒同步更新，選單一開就自動關閉）、`reviewer` 抓到 1 個 High（`PATCH /api/roles/[id]` 只要有 `manage_roles` 權限就能把任一非系統角色的 level 改成 super_admin，形同自我提權），皆已在對應階段直接修正並驗證 build/lint 通過，無回頭重工。完整過程見 `_管理/00_執行紀錄.md` Step 43 條目。以下為原始規劃：
  - **2026-08-27 使用者實機測試後追加修正（項目②真正根因）**：commit/push 上線後使用者實機測試，回報「新增料卡」下拉點任何項目還是會把整個「編輯規則」視窗關掉，不只是清單本身。先後嘗試兩輪修正皆未命中：① 以為是 outside-click 判斷邏輯問題（`dropRef` 寫法比照 `DatePicker.tsx`）；② 以為是 Radix `DismissableLayer` 把 portal 內容誤判成「外部」（補 `DialogContent` 的 `onInteractOutside` + `data-portal-popover` 標記）。兩輪都部署上線後主 session 直接用瀏覽器工具（在使用者協助登入後）實際重現＋用 `document.elementFromPoint()`／檢查 computed `pointer-events` 逐層排查，才找到真正根因：Radix modal `Dialog` 開啟時會把 `document.body` 整個設成 `pointer-events:none`，只在 `DialogContent` 自己身上蓋回 `auto`；`EquipmentQuickPick`/`DatePicker` portal 到 `document.body`、不是 `DialogContent` 的 DOM 子節點，因此繼承到 `none`，選單其實整個點不到，點擊會穿透到 Dialog 自己的遮罩（overlay），觸發 Radix 標準的「點遮罩關閉 Dialog」行為。修法：兩個 portal 內容補上明確的 `pointerEvents: 'auto'`，重新部署後用瀏覽器工具實際登入重現＋確認修正生效（含 `EquipmentQuickPick` 複選+搜尋、`DatePicker` 選日期，皆確認選完後整個 Dialog 仍正常開著）。已補進「已知問題／技術債」章節，避免以後其他 `createPortal` 到 `document.body` 的自訂元件重蹈覆轍。commit `2a1061f`（第二輪嘗試，未解決但保留作為次要防禦層）、`e976656`（真正根因修正）。
  - **2026-08-27 使用者實機測試後第二輪回饋修正**：① 使用統計欄位設定改成批次勾選＋按「套用」才一次送出（原本勾一個就立刻存檔+套用一個，操作不順手），主 session 直接處理（`AnalyticsClient.tsx` 新增 `draftColumns` 本地 state，commit `7b51544`）；② 維修資訊「依料號」查詢頁按「前往廠商頁編輯」只會跳到廠商頁、還要再手動點「編輯規則」，料號一多每筆都要重複這道多餘步驟——改成 `EquipmentRulesPanel` 把 `rule.id` 一併傳給 `onJumpToVendor`，`MaintenanceInfoClient` 依「目標廠商是否已選取」分兩種情況直接開編輯視窗（已選取則規則清單已是最新直接開；未選取則用 `pendingOpenRuleIdRef` 暫存待 `refreshRules` 抓回清單後再開），按鈕文案同步改成「前往編輯此規則」（commit `f7ba3a8`）。兩項改動都用瀏覽器工具實際登入重現＋確認修正生效（欄位設定批次勾選、依料號跳轉直接開編輯視窗且內容正確）。
- **2026-08-27 Step 44：維修資訊「依廠商」規則清單扁平化 + 「依料號」改為全部清單可展開——已執行完成並上線**：使用者反映依廠商頁面「先分料號再分規則」兩層架構會讓同一筆掛多個料號的規則重複出現、依料號頁面要先搜尋才看得到結果也不直覺。先用互動 mockup（`mcp__visualize`）跟使用者對齊設計方向（單欄扁平清單＋單筆/批次展開收合），確認後才動工。CodeGraph+Grep 交叉驗證確認這次要動的 5 個 symbol（`VendorDetailPanel`/`EquipmentSearchPanel`/`EquipmentRulesPanel`/`GENERAL_KEY`/`RuleCard`）唯一呼叫端都侷限在 `src/components/maintenance/` 內，不影響其他功能。依 `frontend`→`tester`→`reviewer` 標準順序委派（純前端，無 API/資料庫異動）：`VendorDetailPanel.tsx` 拿掉「依料號分組」，規則清單改成去重複扁平清單，每筆規則各自可展開/收合，新增「全部展開」（原本只有「全部收合」）；`EquipmentSearchPanel.tsx`+`EquipmentRulesPanel.tsx` 合併成新檔 `EquipmentRuleListPanel.tsx`，一進頁面就列出全部有維修資訊的料號（不用先搜尋），搜尋框變即時篩選，逐項展開才抓資料並快取，新增全部展開/收合，維持唯讀；版面從左右兩欄改成單欄整寬。`tester` 全數驗證通過（含 `pendingFocusRuleIds`/`pendingOpenRuleIdRef` 時序推演、`rulesCache` 有效性）。`reviewer` 直接修正 1 項（`RULE_TYPE_COLOR`/`fmtDateTime`/`emailPrefix` 三處重複定義抽成 `maintenanceFormat.ts` 共用），另提出「未指定料號規則排序」需使用者決定，確認後排到最前面（一併包在同一次提交內）。commit/push main（`5f9c8f6`）。
  1. 維修資訊／廠商刪除：DB 的 `maintenance_rules`/`maintenance_rule_equipment` 已有 `ON DELETE CASCADE`，`src/app/api/maintenance/vendors/[id]/route.ts` DELETE 目前多此一舉擋 409，拿掉阻擋讓 DB cascade 處理；`MaintenanceInfoClient.tsx` 的 `handleDeleteVendor` 改成 ConfirmDialog message 依 `vendor.rule_count` 動態顯示「將一併刪除底下 N 筆規則」
  2. 維修資訊／編輯規則「新增料卡」跑版：根因確認——`EquipmentQuickPick.tsx` 的下拉清單用 `position:fixed`+JS 算座標，但巢狀在 shadcn `DialogContent`（有 `translate-x/y[-50%]` transform）內，transform 祖先會讓 fixed 定位失準（跑版），連帶讓「點外部關閉」的判斷跟畫面看起來的位置對不上。同一個 Dialog 內的 `DatePicker.tsx` 已用 `createPortal` 解決同樣問題，比照套用即可。影響範圍（Grep 交叉驗證）：`RuleFormDialog.tsx`／`PackageListView.tsx`／`ExpandableDocumentList.tsx` 三處都用到 `EquipmentQuickPick`，改動後三處都要驗證
  3. 設備組合：複製功能其實已存在（`DuplicatePackageDialog.tsx`+`pkgApi.duplicate()`+`POST /api/packages/[id]/duplicate`），目前只出現在本部門組合列（`PackageListView.tsx` 的 `!isShared && canEdit` 才顯示），且 API 檢查「來源部門＝呼叫者部門」擋掉跨部門複製。使用者確認沿用既有「先跳小視窗輸入新名稱才建立」UX（不做「先建立再自動彈編輯視窗」）。修法：API 權限放寬為「來源屬於呼叫者部門」或「已分享給呼叫者部門」；`PackageListView.tsx` 在 `isShared` 模式列也顯示複製圖示，改用新的 `edit_own_packages` 權限判斷（不是 `canEdit`/`canShare`）
  4. 系統管理／使用統計：`usage_events` 通用事件表 + `logUsageEvent()` helper（`src/lib/analyticsClient.ts`）+ `getUsageAnalyticsSummary()` 依 `event_type` 分組彙總都已是通用機制，不需改 schema，只要在對應操作點呼叫 `logUsageEvent('新 event_type')`（`PhotoWall.tsx` 的 4 個 `setActiveTab` 分頁按鈕 + `TrackerClient.tsx` 開啟議題詳情處 + `PackageExplorer.tsx` 的 `toggleExpand`）；欄位顯示設定沿用既有通用 `app_settings` key-value 表新增一個 key，`AnalyticsClient.tsx` 加 6 欄 + 右上角欄位設定小圖示
  5. 角色管理／帳號管理：根因確認——`getAssignableRolesData()`（`src/lib/admin.ts`）若角色的 `assignable_role_names` 已明確設定（非 null），會完全忽略 level fallback，導致新建角色不會自動出現在任何既有角色的可指派清單。跟使用者討論釐清 level 系統的實際作用範圍（只影響帳號管理頁的可見範圍/可指派範圍/同步公司帳號按鈕，不影響其他頁面的部門資料隔離）後，確認修法：① `getAssignableRolesData` 對 `level === 'super_admin'` 的角色，不受 `assignable_role_names` 清單限制，永遠回傳全部角色；② 額外發現角色建立後無法修改層級（`PATCH /api/roles/[id]` 目前只支援改 name/department_id），一併補上「事後可修改層級」功能（API 開放 level 欄位 + `RolesManager.tsx` 編輯區塊補層級下拉選單）
...
- **⚠️ 用 `createPortal` 到 `document.body` 的自訂下拉選單，巢狀在 Radix 預設 `modal=true` 的 `Dialog` 內時，Radix 的 `FocusScope` 焦點陷阱＋`RemoveScroll` 捲動鎖定會直接讓它整個點不到／捲不動（2026-08-31 踩坑，`RuleFormDialog.tsx` 的 `EquipmentQuickPick` 新增料卡選單）**：跟下面「pointer-events」那條是同一類 portal-escape 問題，但根因不同、症狀也不同，兩條都要記得檢查。這條的症狀是：搜尋欄位點了完全對焦不到（連 JS 直接呼叫 `.focus()` 都會被立刻拉回去）、清單滾輪完全捲不動，但外層 Dialog 不會被誤觸關閉。原因是 Radix `Dialog` 預設 `modal=true` 會啟用 `FocusScope`（焦點移到 Dialog 外的元素一律強制拉回）+ `RemoveScroll`（只放行 `DialogContent` 內部捲動），兩者都用 DOM containment 判斷「在不在 Dialog 裡」，portal 到 `document.body` 的內容不是 `DialogContent` 的子節點，一律判定為外部而被攔截。**對策**：在該 `<Dialog>` 加上 `modal={false}` 即可關掉這兩個機制，遮罩顯示、點外部關閉等其餘行為不受影響；只需套用在「內部巢狀了 portal-to-body 下拉選單」的那個 Dialog 實例，不要動 `ui/dialog.tsx` 共用元件本身的預設值。
- **⚠️ 用 `createPortal` 到 `document.body` 的自訂下拉選單，巢狀在 shadcn/Radix `Dialog` 內時必須手動補 `pointerEvents: 'auto'`（已踩坑一次，`EquipmentQuickPick.tsx`/`DatePicker.tsx`）**：Radix 的 modal `Dialog` 開啟時會把 `document.body` 整個設成 `pointer-events: none`，只在 `DialogContent` 自己身上明確蓋回 `auto`。任何用 `createPortal(..., document.body)` 渲染、且不是 `DialogContent` DOM 子節點的自訂內容（例如為了避免被 `overflow-hidden` 裁切而 portal 出去的下拉選單/日期選擇器），會直接繼承 body 的 `none`，導致整個選單其實點不到——使用者點擊會「穿透」到 Dialog 自己的半透明遮罩（overlay），而點遮罩在 Radix 裡就是標準的「關閉這個 Dialog」觸發方式，造成「巢狀選單一點就把整個編輯視窗關掉」的詭異現象（且不會有任何 JS 錯誤、`z-index` 看起來也完全正常，只有直接用瀏覽器 `document.elementFromPoint()`/檢查 computed `pointer-events` 才能看出破綻，肉眼排查程式碼邏輯完全看不出來）。**對策**：這類 portal 內容的最外層固定要手動加 `style={{ ..., pointerEvents: 'auto' }}`；新增任何 `createPortal` 到 `document.body` 的元件時，若有可能被用在 Dialog 內，一律加上這行，不要等出事才補。
