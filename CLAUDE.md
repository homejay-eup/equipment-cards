...
- **進行中**：Step 30（文件（規格書）管理架構重整：資料正規化 + Google Drive Service Account，完整規格見 plan 檔案 `C:\Users\EupUser\.claude\plans\merry-imagining-pumpkin.md`）
  ...
  - 🔄 **2026-07-13 使用者提出：文件連結功能需要重新規劃使用方式，目前只是「堪用」，還沒達到目標**——現況已達成的是「共用雲端硬碟當儲存後端，業務在外部也能透過料卡網站看到並點開文件」這個最基本的骨架，但以下幾個實務使用情境完全還沒規劃，**下一個 session 開場請先回到腦力激盪階段跟使用者逐一討論收斂，不要直接動手實作**：
    1. **批次上傳**：目前一次只能選一個檔案、指定一組 type/name 上傳，沒有「一次選多個檔案、自動對應到不同料號或不同文件類型」的批次流程
    2. **批次變更舊檔（換版）**：「更新版本」目前是逐筆針對單一文件操作，沒有批次替換多筆舊文件的方式
    3. **重複上傳情境**：已有的「上傳同名文件跳提示」只處理單筆上傳當下的精確同名比對，批次情境、相似但非完全同名（例如版本號不同）等情況都還沒設計
    4. **樂觀顯示（避免感覺延遲）**：目前上傳/掛載/移除多半要等 API 回應才更新畫面（或暫存到按儲存才生效），使用者體感可能覺得慢，要不要加樂觀更新（先顯示、失敗再回滾）沒討論過
    5. **Drive 內檔案的顯示/組織方式**：目前所有上傳檔案都在同一個平面資料夾「產品規格書 (上傳)」，沒有依料號分資料夾，業務或管理員如果想直接在 Google Drive 側瀏覽（不透過網站）會看不出歸屬——要不要改成依料號建子資料夾、或用檔名前綴料號，需要跟使用者討論利弊（子資料夾管理更直覺，但檔案跨料號共用時歸屬到哪個資料夾會是新問題；目前 172 筆舊資料 + 新上傳都在同一層，改動涉及既有檔案是否要一併搬移）
...
**Step 30 文件連結功能——重新規劃使用方式（腦力激盪階段，不要直接動手實作）**

上一個 session 結束時 context window 已達 55%，使用者主動要求收尾交接。Step 30 的基礎機制（Drive 當儲存後端、正規化資料表、上傳/挑選/移除/更新版本 API、CardFormDialog 文件區塊、Drive 刪除權限問題修正）都已完成並上線驗證過，但使用者實際使用後認為**目前只是「堪用」**，還沒達到他要的完整使用體驗。

**開場請先讀** `_管理/00_執行紀錄.md` 最後幾筆條目（Step 30 相關的所有條目，尤其「Drive 刪除永久失敗根因診斷」）跟本檔案「目前進度」Step 30 區塊，掌握現況後，**直接跟使用者逐一討論以下 5 個開放問題**（本檔案「目前進度」已列出，這裡不重複貼），一次只問一個，列選項優缺點讓使用者收斂，討論完才決定要不要動工：
1. 批次上傳
2. 批次變更舊檔（換版）
3. 重複上傳情境（含批次情境、相似非完全同名）
4. 樂觀顯示（減少感覺延遲）
5. Drive 內檔案組織方式（要不要依料號分資料夾）

不確定使用者這次想從哪個問題先討論，可以直接問。
