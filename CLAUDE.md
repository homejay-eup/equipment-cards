...
- **2026-08-26 Step 43：5 項需求已對焦，待 CodeGraph 排查影響範圍與寫規格**：
  1. 維修資訊／廠商刪除：廠商底下尚有規則時，把原生 `alert()` 阻擋改成符合網站風格、不阻擋的提示，實際刪除時連同底下規則與規則掛載的料號關聯一起清除
  2. 維修資訊／編輯規則「新增料卡」：修正清單彈窗跑版 + 點擊彈窗外部導致清單消失的 bug
  3. 設備組合：新增「複製別人分享給我的組合」——複製後固定歸屬到使用者目前登入部門，自動彈出編輯視窗改名稱/內容
  4. 系統管理／使用統計：新增 5 項頁面切換/瀏覽次數統計（任務板切換、任務卡議題打開、人為配件報價切換、文件管理切換、設備組合切換＋瀏覽、維修資訊切換），透過表格右上角小圖示做全域欄位顯示設定（存資料庫，所有管理員看到一致）
  5. 角色管理／帳號管理：新增的自訂角色（「wei wei」「wei」）在帳號管理的角色下拉選單選不到，需排查根因（bug）
