...
**主題**：「我的關注」／「設備套餐」頁面「群組(套餐)組合情境太多太亂、難維護」的架構優化討論。**目前仍在腦力激盪階段，尚未進行 CodeGraph 調查或執行任何程式碼變更**。完整討論脈絡（考慮過的方案、排除原因、標籤維度表格）已記錄於 `_管理/00_方案紀錄.md`「[2026-08-12] 群組/套餐管理架構優化」條目，新 session 開場請先讀那一條再接續討論。

**已收斂的方向（3 件事，尚未執行）**：
1. 套餐/群組新增**自由標籤**分類（比照既有 `category_subfilter_tags` 架構延伸，不重新設計標籤系統）
2. 善用既有「複製套餐」功能取代模組化自動組裝（使用者確認選項會變動＋內容相似但不全同，模組化不划算）
3. 補齊「設備套餐」頁面缺的**跨套餐批次替換**功能（「我的關注」的替換料卡彈窗已有「此料卡同時存在於」勾選多群組一次套用的功能，套餐目前沒有，範圍明確可直接排入執行）

**標籤分類維度**（已收斂，完整表格見 00_方案紀錄.md）：客戶、車款(含油/油電)、大小車/輪數、安裝位置(車頭車尾/室內外/左右)、有無子車(母/子車頭)、業務性質(新單/維護/進貨/補助案)、主機型號、鏡頭方案(幾鏡)、套餐狀態(現行/停用)。另有備註欄（新欄位，裝一次性客製細節，判斷原則：常重複出現→標籤；一次性→備註）。

**下一步待確認（新 session 開場先問這個）**：
1. 剛從使用者提供的 CRM 套餐清單參考資料（`C:\Users\EupUser\Desktop\Web App for Develop - CRM_DB_FO\_管理\EUPCRM_出貨品項套餐清單.xlsx`，內部真實套餐命名歷史 3033 行/55 種主機類別/321 個套餐）發現兩個新維度：「**附加功能/選配模組**」（ADAS/DMS/胎壓/溫控/酒測/疲勞駕駛/倒車雷達/刷卡機/BSD/到站播報器）跟「**車輛用途/載運內容**」（廢棄物/毒化物/土方/冷藏/貨運/遊覽車/水箱車/救護車）——**已詢問使用者是否納入，尚未收到回覆**
2. 標籤畫面呈現方式（套餐/群組列表要不要顯示標籤、篩選器 UI 長什麼樣子）尚未討論
3. 確認完標籤維度＋畫面呈現後，才進入 CodeGraph blast radius 調查（預期涉及 `GroupsPanel.tsx`、`PackagesClient.tsx`/`PackageExplorer.tsx`/`PackageListView.tsx`/`EquipmentListView.tsx`，可能新增標籤相關資料表或延伸既有 `category_subfilter_tags` 架構），再委派 `data`/`frontend` 執行；跨套餐批次替換功能可以獨立先做（範圍已明確，不受標籤討論進度影響）
