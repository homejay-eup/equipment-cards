...
// 從 source_group_id 指向的「我的關注」組合重新對齊：
// 用來源目前名稱 + group_items 整個覆蓋設備組合的 name + package_items，更新 source_synced_at
// 權限：edit_own_packages + 必須是來源組合的擁有者（單向手動觸發，僅限個人組合本人操作）
export async function POST(_req: NextRequest, { params }: { params: { id: string } }) {
  ...
  try {
    ...
    if (!pkg.source_group_id) {
      return NextResponse.json({ error: '此設備組合未連結來源' }, { status: 400 })
    }
    ...
    if (!group) {
      return NextResponse.json({ error: '來源組合已不存在' }, { status: 404 })
    }
    // 僅限來源組合的擁有者本人可觸發對齊
    ...
    if (updateError) {
      if (updateError.code === '23505') {
        return NextResponse.json({ error: '對齊後的名稱與同部門其他組合重複' }, { status: 409 })
      }
      throw updateError
    }
