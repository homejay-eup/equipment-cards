...
import { EquipmentCard, DetailPhoto } from '@/types/equipment'

const CATEGORIES = ['主機', '鏡頭', '螢幕', '天線', '儲存媒體', '線材', '配件', '耗材', '工具', '國外設備']
...
export default function CardFormDialog({ mode, card, open, onClose }: Props) {
  ...
  async function handleAddDetail(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (!equipId) { setError('請先填入料號'); return }

    setUploading(true)
    setError(null)
    try {
      for (const file of Array.from(files)) {
        const type = `detail_${Date.now()}`
        const result = await uploadPhoto(file, equipId, type)
        ...
        <div className="overflow-y-auto flex-1 px-6 py-4 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              ...
              <select
                value={form.status}
                onChange={e => set('status', e.target.value as 'active' | 'discontinued')}
                className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm bg-white focus:outline-none focus:ring-2 focus:ring-blue-400"
              >
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {detailPhotos.map(photo => (
                <div key={photo.public_id} className="relative group w-20 h-20 rounded-lg overflow-hidden border border-gray-200 bg-gray-50 flex-shrink-0">
                  <Image src={photo.url} alt="細節照片" fill className="object-cover" />
                  <button
                    type="button"
                    onClick={() => handleDeleteDetail(photo.public_id)}
                    disabled={isBusy}
