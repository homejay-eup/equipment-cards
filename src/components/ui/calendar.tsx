...
// Cell size fixed at 2.25rem (36px) — v3-compatible, no CSS variable dependency
const CELL = "w-9 h-9"
    ...
    <DayPicker
      ...
      classNames={{
        ...
        button_previous: cn(
          ...
          `${CELL} p-0 select-none aria-disabled:opacity-50`,
      ...
      components={{
        ...
        Chevron: ({ className, orientation, ...props }) => {
          if (orientation === "left") return <ChevronLeftIcon className={cn("w-4 h-4", className)} {...props} />
          if (orientation === "right") return <ChevronRightIcon className={cn("w-4 h-4", className)} {...props} />
          return <ChevronDownIcon className={cn("w-4 h-4", className)} {...props} />
        },
        DayButton: ({ ...props }) => <CalendarDayButton locale={locale} {...props} />,
        WeekNumber: ({ children, ...props }) => (
          <td {...props}>
            <div className="flex w-9 h-9 items-center justify-center text-center">{children}</div>
          </td>
        ),
        ...components,
      }}
...
function CalendarDayButton({
  ...
  return (
    <Button
      ...
      className={cn(
        "relative isolate z-10 flex w-9 h-9 min-w-[2.25rem] flex-col gap-1 border-0 leading-none font-normal",
